...
function initializeTopics() {
    const savedTopics = localStorage.getItem('learningAcademyTopics');
    if (savedTopics) {
        topics = JSON.parse(savedTopics);
    } else {
        topics = [...defaultTopics];
        localStorage.setItem('learningAcademyTopics', JSON.stringify(topics));
    }
}
...
function showPillarDetail(pillar) {
    ...
    // Populate Topics as Cards
    const topicsList = document.getElementById('detailTopics');
    const pillarTopics = topics.filter(t => t.pillarId === pillar.id);
    topicsList.innerHTML = pillarTopics.map((topic, idx) => 
        `<div class="topicCard" onclick="showItemDetail('topic', '${topic.id}', ${idx}, '${escapeHtml(topic.title)}')">${escapeHtml(topic.title)}</div>`
    ).join('');

    // Populate SOPs with clickable items
    const sopsList = document.getElementById('detailSOPs');
    if (sopsList) {
        sopsList.innerHTML = pillar.sops.map((sop, idx) => 
            `<div class="sopItem" onclick="showItemDetail('sop', '${pillar.id}', ${idx}, '${escapeHtml(sop)}')"><strong>${sop}</strong></div>`
        ).join('');
    }

    // Populate Materials with clickable items
    const materialsList = document.getElementById('detailMaterials');
    if (materialsList) {
        materialsList.innerHTML = pillar.materials.map((material, idx) => 
            `<div class="materialItem" onclick="showItemDetail('material', '${pillar.id}', ${idx}, '${escapeHtml(material)}')"><strong>${material}</strong></div>`
        ).join('');
    }
...
function showItemDetail(type, itemId, index, title) {
    // For topics, itemId is the topic.id
    ...
    if (type === 'topic') {
        ...
        if (topic) {
            ...
            document.getElementById('contentParagraph').textContent = topic.description || topic.content || 'No content available';
        } else {
            document.getElementById('contentParagraph').textContent = 'Topic content not found';
        }
    } else {
        // Generate sample content based on type and title
        const keyPoints = getItemKeyPoints(type, title);
        const content = getItemContent(type, title);
...
function submitAddItem(event) {
    ...
    const content = document.getElementById('itemContent').value || 'Not specified';

    // Only handle topics for now - save to localStorage
    if (itemType === 'topic' && currentPillar) {
        ...
        // Update the UI - refresh the pillar detail page
        showPillarDetail(currentPillar);

        const message = `✅ Topic Added Successfully!\n\nTitle: ${itemName}\nDescription: ${description}`;
        alert(message);
    } else {
        const message = `✅ New ${itemType.toUpperCase()} Added!\n\nName: ${itemName}\nDescription: ${description}\n\nThis item has been successfully created.`;
        alert(message);
    }
...
function submitAddPillar(event) {
    ...
    const smeEmail = document.getElementById('smeEmail').value;
    const topicCount = document.getElementById('topicCount').value || '4';

    const message = `✅ New Pillar Request Submitted!\n\nPillar Name: ${pillarName}\nDescription: ${description}\nSME: ${smeName}\nEmail: ${smeEmail}\nTopics: ${topicCount}\n\nYour request has been submitted to the administrator for review.`;
